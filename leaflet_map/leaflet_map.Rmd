...
labels_ambient <- achievement_levels %>%
  ...
  filter(str_detect(metric, "so2")) %>%
  add_row(metric = c("so2_3yr", "so2_1yr"), 
  ...
  mutate(labels_full = factor(labels_full, levels = unique(labels_full)),
         metric_nice = if_else(metric == "so2_3yr", "1-Hr", "Annual")) %>%
...
stations <- read_sf(here("out/so2_caaqs.geojson")) %>%
  ...
  mutate(p_az = airzone,
         p_station = site,
         p_station_id = paste0(site, "_", str_remove(metric, "so2_"))) %>%
  mutate(popup = popup_caaqs(
    ., type = "station", 
    units = units_html,
    metric_name = paste0("SO<sub>2</sub> Metric (", metric_nice, ")"),
    standard_name = paste0("SO<sub>2</sub> Air Quality Standard (", metric_nice, ")")))
...
az <- st_read(here("./out/so2_airzone.geojson")) %>%
  ...
  mutate(p_az = airzone,
         ...
         p_station_id = paste0(rep_station_id, "_", str_remove(metric, "so2_"))) %>%
  mutate(popup = popup_caaqs(
    ., type = "region", 
    units = units_html,
    metric_name = paste0("SO<sub>2</sub> Metric (", metric_nice, ")"),
    standard_name = paste0("SO<sub>2</sub> Air Quality Standard (", 
                           metric_nice, ")")), 
...
pal_az_1yr <- filter(labels_ambient, metric == "so2_1yr") %>%
  colorFactor(palette = .$colour, levels = .$labels_full)

pal_az_3yr <- filter(labels_ambient, metric == "so2_3yr") %>%
  colorFactor(palette = .$colour, levels = .$labels_full)
...
markers_1yr <- markers
markers_1yr$iconUrl <- markers_1yr$iconUrl[stations$metric == "so2_1yr"]
markers_3yr <- markers
markers_3yr$iconUrl <- markers_3yr$iconUrl[stations$metric == "so2_3yr"]
...
m %>%
  ...
  addPolygons(data = filter(az, metric == "so2_1yr"), group = "so2_1yr",
  ...
  # Add station markers
  addMarkers(data = filter(stations, metric == "so2_1yr"), group = "so2_1yr",
  ...
  # Legend for so2_1yr
  addLegend("bottomright", group = "so2_1yr",
            data = filter(az, metric == "so2_1yr"),
            # Ensure we get all levels
            colors = filter(labels_ambient, metric == "so2_1yr") %>%
              pull(colour), 
            labels = filter(labels_ambient, metric == "so2_1yr") %>%
              pull(labels_full),
            opacity = 1, 
            title = htmltools::HTML("<h3>Air Zones</h3><h4>SO<sub>2</sub> Air Quality Standard (annual)</h4>")) %>%
  
   # Custom legends for markers
  addLegend("bottomleft", group = "so2_1yr",
            colors = filter(labels_mgmt, metric == "so2_1yr") %>% pull(colour) %>% rev(),
            className = "info legend solid",
            labels = filter(labels_mgmt, metric == "so2_1yr") %>% pull(labels_full) %>% rev(),
            opacity = 1,
            title = htmltools::HTML("<h3>SO<sub>2</sub> Monitoring Stations</h3><h4>SO<sub>2</sub> Metric (annual)</h4>"))
```
...
m %>%
  # Add airzone polygons
  addPolygons(data = filter(az, metric == "so2_3yr"), group = "so2_3yr",
              color = "white", weight = 2, opacity = 1, fillOpacity = 0.7,
  ...
  # Add station markers
  addMarkers(data = filter(stations, metric == "so2_3yr"), group = "24-so2_3yr",
  ...
  # Legend for polygons 
  addLegend("bottomright", group = "so2_3yr",
            data = filter(az, metric == "so2_3yr"),
            colors = filter(labels_ambient, metric == "so2_3yr") %>%
              pull(colour), 
            labels = filter(labels_ambient, metric == "so2_3yr") %>%
              pull(labels_full),
            opacity = 1, 
            title = htmltools::HTML("<h3>Air Zones</h3><h4>SO<sub>2</sub> Air Quality Standard (1 Hr)</h4>")) %>%
   # Custom legends to override order
  addLegend("bottomleft", group = "so2_3yr",
            colors = filter(labels_mgmt, metric == "so2_3yr") %>% pull(colour) %>% rev(),
            className = "info legend solid",
            labels = filter(labels_mgmt, metric == "so2_3yr") %>% pull(labels_full) %>% rev(),
            opacity = 1,
            title = htmltools::HTML("<h3>SO<sub>2</sub> Monitoring Stations</h3><h4>SO<sub>2</sub> Metric (1 Hr)</h4>"))
```
